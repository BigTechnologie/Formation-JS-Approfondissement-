// Récupère la barre de recherche // Récupère l'élément HTML avec l'ID 'search' (la barre de recherche)
let search = document.getElementById('search');

// Récupère la zone où les résultats vont s'afficher // Récupère l'élément HTML avec l'ID 'results' (où les résultats vont s'afficher)
let results = document.getElementById('results');

// Modèle de carte pour afficher un film // Modèle HTML pour afficher une carte de film
// Les mots TITLE, RESUME, DATE, LINK et IMG seront remplacés par les vraies données // Les placeholders TITLE, RESUME, DATE, LINK et IMG seront remplacés dynamiquement
let card = `
...
// Variable pour gérer le délai entre chaque frappe (debounce) // Variable pour gérer le délai avant de lancer la recherche (debounce)
let timeout;

// Récupère l'élément pour l'affichage du loader // Récupère l'élément HTML avec l'ID 'loading' (pour afficher un indicateur de chargement)
let loading = document.getElementById('loading');

// Ajoute un écouteur d'événement sur la barre de recherche // Dès qu'une touche est pressée, on déclenche une action
// Quand l'utilisateur tape dans la barre de recherche
search.addEventListener('keydown', () => {
    // Affiche le loader pendant que la recherche est en cours
    loading.style.display = 'block';

    // Vide les résultats précédents
    results.innerHTML = '';

    // Annule un éventuel timeout précédent pour éviter plusieurs appels API // Annule le timeout précédent (si l'utilisateur tape vite) 
    clearTimeout(timeout);

    // Lance la recherche après 1 seconde // Cela évite de faire une requête à chaque touche pressée
